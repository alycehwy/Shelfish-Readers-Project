    ...
    <article class="editbook_body">
        ...
        <?php
            if($_SERVER['REQUEST_METHOD'] == "POST"){
                if($_POST['b_title']!="" && $_POST['b_author']!="" && $_POST['b_price']!="" && $_POST['b_description']!="" && $_POST['b_keywords']!="" && $_POST['b_likes']!=""){
                    $updatebook = "UPDATE book_tb SET b_title='".$_POST['b_title']."',b_author='".$_POST['b_author']."',b_price='".$_POST['b_price']."',b_description='".$_POST['b_description']."',b_keywords='".$_POST['b_keywords']."' ,b_like='".$_POST['b_likes']."'WHERE b_id=".$_SESSION['bookData']['b_id'];
                    if($dbConection -> query($updatebook) === true){
                        $selectuser = "SELECT * FROM book_tb WHERE b_id = ".$_SESSION['bookData']['b_id'];
                        $result = $dbConection -> query($selectuser);
                        $row = $result->fetch_assoc();
                        $dbConection -> close();
                        echo "<script>alert('Edit Success')</script>";
                        if($_SESSION['bookData']['b_type'] == '1'){
                            unset($_SESSION['bookData']);
                            header("Refresh:0.01; url=http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/books", true);
                        }
                        else{
                            unset($_SESSION['bookData']);
                            header("Refresh:0.01; url=http://".$_SERVER['HTTP_HOST'].dirname($_SERVER['PHP_SELF'])."/bookssec", true);
                        }
                    }
                }
                else{
        ...
        <form method="POST" action="<?php echo parse_url($_SERVER['REQUEST_URI'],PHP_URL_PATH) ?>">
            <?php
                ...
                echo "<input type='text' class='form-control' name='b_likes' value='".$_SESSION['bookData']['b_like']."' required/></div>";
